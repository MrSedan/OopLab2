...
\begin{document}
  ...
  \texttt{DbContextOptions<AppDbContext>}, который передается в базовый класс. Entity Framework Core выступает в роли ORM, связывающей объекты предметной области с таблицами базы данных, а также обеспечивает выполнение запросов и сохранение изменений в PostgreSQL.
  ...
  В приложении реализована CRUD-модель работы с данными. Для объектов \texttt{Server} и \texttt{PC} предусмотрены следующие операции:

  \begin{itemize}
    \item[--] \textbf{Create} --- создание новой записи. Для серверов это метод \texttt{CreateServer}, который принимает данные из запроса, формирует объект \texttt{Server} и передает его в сервисный слой для добавления в базу данных. Для компьютеров используется аналогичный сценарий через метод создания объекта \texttt{PC}.
    \item[--] \textbf{Read} --- чтение данных. Метод \texttt{GetAllServers} возвращает список всех серверов, сохраненных в системе, а дополнительные методы получения информации возвращают сведения по выбранному объекту. Для компьютеров чтение реализовано по тому же принципу.
    \item[--] \textbf{Update} --- обновление существующей записи. Метод \texttt{UpdateServer} находит сервер по идентификатору, изменяет его характеристики и сохраняет новые значения в базе данных. Для класса \texttt{PC} обновление выполняется аналогичным образом.
    \item[--] \textbf{Delete} --- удаление записи. Метод \texttt{DeleteServer} удаляет сервер по уникальному идентификатору. Для объектов \texttt{PC} удаление выполняется по той же схеме через сервисный слой.
  \end{itemize}

  Сервисный слой \texttt{ComputerLabService} использует \texttt{AppDbContext} для добавления, изменения, удаления и выборки сущностей, а затем сохраняет изменения в базе данных.
